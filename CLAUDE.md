...
- **Forecast arrays are filtered to what has not already finished.**
  ...
  those cases are testable.- **`weatherPrimaryCoded` decides the condition, not the icon name.**
  ...
  The plant app at `/` is out of scope for this and keeps its own emoji.
...
- **Station geography is not a reading, and must not share its TTL.** The tide
  ...
  all — the concurrency fix underneath it never got a chance to run. It is
  cached for a day (`TTL.stations`) now.
- **`left()` reports the real remaining budget, including zero.** It used to be
  ...
  it never saw less. `spend()` is where the per-request floor belongs.
